...
volatile File_handle_t* start_handle,*end,*current; //file queue

volatile Special_Dirs_t curr_dir, temp_curr_dir, root_dir;

volatile Directory_t current_;

volatile HBA_PORT* curr_port;

volatile Disk_dev_t* curr_disk;

volatile ahci_t* curr_ahci;

volatile uint32_t sectors;

volatile uint32_t bytemap_off;

volatile uint32_t bytemap_end;

volatile File_t curr_file;

volatile uint32_t fsbuf;

volatile size_t header_data=1024-sizeof(File_Header_t);
