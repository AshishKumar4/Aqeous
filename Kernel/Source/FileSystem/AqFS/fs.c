...
int file_read(uint32_t* buffer, uint32_t offset, uint32_t size, File_handle_t* handle)
{
	return file_readTM(buffer, offset, size, handle->full_path);
}
...
int file_GetSize(File_handle_t* handle)
{
	return file_size(handle->name);
}

int file_vfsClose(File_handle_t* handle)
{
	file_closeOGP(handle->full_path);
}

uintptr_t file_vfsLoad(char* full_path)
{
	file_loadOGP(full_path);
	return (uintptr_t)file_searchOGP(full_path);
}
...
int AqFS_init()
{
	...
	if(sectors >= 0x100000) sectors = 0x100000;
...
void AqFS_burn()
{
	...
	sectors=info->lba_capacity;
	printf("\nTotal Sectors: %x\n",sectors);
	if(sectors >= 0x10000) sectors = 0x10000;
	start_off=10240;
	bytes=sectors;
	off=(bytes/512)/512;
	off += 16;
...
void AqFS_burnTest()
{
	uint32_t *tlff = (uint32_t*)phy_alloc4K();
	memset_fast((void*)tlff, 4284, 1024);
	sec_write_static(curr_port, 100, 1, (uint32_t)tlff);
	memset_fast((void*)tlff, 0, 1024);
	sec_read_static(curr_port, 100, 1, (uint32_t)tlff);
	for(int i = 0;i < 10; i++)
	{
		printf(" {%d}", tlff[i]);
	}
	...
	*boot_ptr = 0xAA;
	...
	uint32_t* buff = (uint32_t*)kmalloc(4096);
	memset_fast((void*)buff, 4284, 1024);
 	write(curr_port, 100, 1, (uint32_t)buff);
	memset_fast((void*)buff, 0, 1024);
	read(curr_port, 100, 1, (uint32_t)buff);
	for(int i = 0;i < 10; i++)
	{
		printf("\n%d", buff[i]);
	}

	identity=(Identity_Sectors_t*)(buf + 436);
	if(strncmp(identity->name, "AqFS472",7))
	{
		printf("Filesystem Not supported/Disk not partitioned Correctly\n");
		//AqFS_burn();
