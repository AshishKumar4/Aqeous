...
{
  return (Water_OperatorObject_t*)HashTable_OpObj[hash];
}
...
Water_OperatorToken_t* Water_GetOpToken(const char* str, int sz)
{
  ...
  Water_OperatorObject_t* opObj = (Water_OperatorObject_t*)HashTable_OpObj[opHash];
//  if(!opClass) return 0;
...
{
  Water_OperatorObject_t* opObj = (Water_OperatorObject_t*)HashTable_OpObj[hash];
//  if(!opClass) return 0;
  Water_OperatorToken_t* obj = (Water_OperatorToken_t*)Water_alloc(sizeof(Water_OperatorToken_t));
  obj->op = opObj;
  obj->hash = hash;
...
int Water_KeywordHandler(Water_Token_t* TokenStart)   // If any keywords, Execute them
{
  ...
  for(;tmpToken;)
  {
    ...
    {
      Water_Keyword_t* tmp = (Water_Keyword_t*)tmpToken;
      tmp->func(tmpToken);
    }
...
void Water_init()
{
  ...
  HashTable_OpObj = (uintptr_t*)pmem_4k(1);
  HashTable_keywords = (uintptr_t*)pmem_4k(2);
  HashTable_Vars = (uintptr_t*)pmem_4k(2);

  Water_NewOperatorObj("=", 1, 1, 0, Water_opEqual);
  ...
  *ValStackTop = NULL;
  ...
  Water_NewKeyword("int", 3, WATER_INT, WaterKeyword_int);
}
...
int Water_MakeTokenList(const char* sentence, Water_Token_t* TokenStart)
{
  ...
  for(;tmpStr[0] != '\0';)
  {
    ...
    else if(tmpStr[0] != ' ')   // Must be an Operator
    {
      ...
      // Get the Token
      Water_Token_t* token = (Water_Token_t*)Water_GetOpToken(tmpStr, i);
  ...
  for(;tmpStr[0] != '\0';)
  {
    if(isalpha(tmpStr[0]))    // If First letter is alphabet
    {
      ...
      else  // Must be a Variable or a Function
      {
        //printf("\n-=->%d", hash);
        token = (Water_Token_t*)Water_GetBasicTypeToken_vh(hash);   // Get the token for variable/function
        if(!((Water_BasicTypeToken_t*)token)->var)
        {
          printf("\n%s Variable/Function not defined!", tmpStr);
          strncpy(((Water_BasicTypeToken_t*)token)->name, tmpStr, i);
        //  return 1;
      ...
      //  printf("\n<%s>", ((Water_BasicTypeObject_t*)token)->name);
      }
      tmpToken->NextToken = token;    // Put the Token into the Token List.
      token->BackToken = tmpToken;
      token->NextToken = EndToken;
    ...
    {
      Water_Token_t* token;
      token = (Water_Token_t*)Water_GetOpToken(tmpStr, 1);   //  Execute the Internal Expression RECURSIVELY
      if(!token)
      {
        printf("\n%s Internal Expression Error", tmpStr);
        return 4;
    ...
    else if(tmpStr[0] != ' ')   // Must be an Operator
    {
      ...
      // Get the Token
      Water_Token_t* token = (Water_Token_t*)Water_GetOpToken(tmpStr, i);   //  Execute the Internal Expression RECURSIVELY
