...
uint32_t kmalloc_int(uint32_t sz, int align)
{
 ...
 //   printf("\ntsz4: %x sz: %x pgs: %x", tsz4, sz, pgs);
    ...
    printf("\n Something went wrong, Memory Management Failure");
    while(1);
    return 0;
}//

void kfree(uint32_t* ptr) //New!
{
  uint32_t loc = (uint32_t)ptr;
  uint32_t loff = loc%4096;
  uint32_t frame_4k_aligned = loc - loff;
  uint32_t frame = frame_4k_aligned / 4096;
  ...
  if(((*((uint32_t*)(loff + 4092))) & (42847 << 16)) == (42847 << 16))  //The memory is inside one page.
  {
    uint32_t* i = ptr;
    for(; !((*i & 0xffff0000) == 42847); i++);
    {
      *i = 0;
    }
    uint32_t sz = (((*i) & 0x0000ffff) >> 16);
    *i = 0;
    // TODO: Get the block, deduct the size from the block as well as the last strip of the page. 
    Block->used -= sz;
    i = (loc + 4092);
    uint16_t* last_strip = (uint16_t*)(loc + 4092);
    --*last_strip;
  }
  else  //The memory is spread over several pages.
  {
    uint32_t iid = Block->id;
    MemMap_t* tmpBlock = Block;

    for(uint32_t i = loc; tmpBlock->id == iid; ++i, ++tmpBlock)
    {
      //Optimize this peice of code!
      tmpBlock->id = 0;
      tmpBlock->used = 0;
      tmpBlock->page = 0;
