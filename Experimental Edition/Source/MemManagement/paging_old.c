...
inline void Switch_to_system_dir()
{
  if(!in_system_dir)
  {
    ...
    if(multitasking_ON)
    {
      ((task_t*)current_task)->pgdir = (uint32_t)system_dir;
    }
    else
    {
        in_system_dir=1;
...
inline void Switch_back_from_System()
{
  if(in_system_dir)
  {
    if(multitasking_ON)
    {
      _cur_directory = (pdirectory*)((task_t*)current_task)->main_pgdir;
      ((task_t*)current_task)->pgdir = (uint32_t)_cur_directory;
    }
    else
    {
